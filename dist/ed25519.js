...
exports.ed25519Verify = exports.ed25519Sign = exports.ed25519KeypairFromEntropy = void 0;
...
function crypto_sign_keypair(pk, sk) {
    ...
    const p = [gf(), gf(), gf(), gf()];
    let i;
    (0, sha512_js_1.sha512internal)(d, sk, 32);
    d[0] &= 248;
    d[31] &= 127;
    d[31] |= 64;
    scalarbase(p, d);
...
function crypto_sign_open(m, sm, n, pk) {
    ...
    for (i = 0; i < 32; i++)
        m[i + 32] = pk[i];
    (0, sha512_js_1.sha512internal)(h, m, n);
    reduce(h);
    scalarmult(p, q, h);
...
function crypto_sign(sm, m, n, sk) {
    ...
    for (i = 0; i < 32; i++)
        sm[32 + i] = d[32 + i];
    (0, sha512_js_1.sha512internal)(r, sm.subarray(32), n + 32);
    reduce(r);
    scalarbase(p, r);
    pack(sm, p);
    for (i = 32; i < 64; i++)
        sm[i] = sk[i];
    (0, sha512_js_1.sha512internal)(h, sm, n + 64);
    reduce(h);
    for (i = 0; i < 64; i++)
        x[i] = 0;
    for (i = 0; i < 32; i++)
...
function ed25519KeypairFromEntropy(seed) {
    ...
    if (errU8 !== null) {
        return [nkp, (0, err_js_1.addContextToErr)(errU8, "seed is invalid")];
    }
...
exports.ed25519KeypairFromEntropy = ed25519KeypairFromEntropy;
...
function ed25519Sign(msg, secretKey) {
    ...
    if (errU8 !== null) {
        return [nu8, (0, err_js_1.addContextToErr)(errU8, "inputs are invalid")];
    }
