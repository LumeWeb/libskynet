...
test("encodeAndDecodeU64", () => {
    ...
    for (let i = 0; i < tests.length; i++) {
        const [enc, errEU64] = (0, encoding_1.encodeU64)(tests[i]);
        expect(errEU64).toBe(null);
        const [dec, errDU64] = (0, encoding_1.decodeU64)(enc);
...
test("bufToB64", () => {
    ...
    for (let i = 0; i < tests.length; i++) {
        const result = (0, encoding_1.bufToB64)(tests[i].trial);
...
test("bufToHexAndBufToHex", () => {
    ...
    // Test hexToBuf
    for (let i = 0; i < tests.length; i++) {
        const result = (0, encoding_1.bufToHex)(tests[i].trial);
        expect(result.length).toBe(tests[i].result.length);
        for (let j = 0; j < result.length; j++) {
            expect(result[j]).toBe(tests[i].result[j]);
        }
    ...
    // Test bufToHex.
    for (let i = 0; i < tests.length; i++) {
        const [result, err] = (0, encoding_1.hexToBuf)(tests[i].result);
        ...
        const [result2, err2] = (0, encoding_1.hexToBuf)(tests[i].result.toUpperCase());
    ...
    for (let i = 0; i < invalids.length; i++) {
        const [, err] = (0, encoding_1.hexToBuf)(invalids[i]);
        if (err === null) {
