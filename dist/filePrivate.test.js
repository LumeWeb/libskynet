...
const filePrivate_js_1 = require("./filePrivate.js");
...
test.each([
    ...
    expect((0, filePrivate_js_1.getPaddedFileSize)(inputFileSize)).toBe(outputFileSize);
});
...
test("testEncryptFileSmall", () => {
    ...
    const [seed, errSPTS] = (0, seed_js_1.seedPhraseToSeed)(seedPhrase);
    ...
    const [encryptedData, errEF] = (0, filePrivate_js_1.encryptFileSmall)(seed, inode, revision, metadata, fileData);
    ...
    const encryptedDataHash = (0, sha512_js_1.sha512)(encryptedData);
    // Attempt to decrypt the file.
    const [recoveredMetadata, recoveredFileData, errDF] = (0, filePrivate_js_1.decryptFileSmall)(seed, inode, encryptedData);
    expect(errDF).toBe(null);
    // Check that decryption did not change the encrypted data.
    const encryptedDataHash2 = (0, sha512_js_1.sha512)(encryptedData);
    ...
    const [encData2, errEFS] = (0, filePrivate_js_1.encryptFileSmall)(seed, inode, revision + 1n, metadata, fileData);
    ...
    const [spd, errGSPD2] = (0, seed_js_1.generateSeedPhraseDeterministic)("a different seed");
    expect(errGSPD2).toBe(null);
    const [seed2, errSPTS2] = (0, seed_js_1.seedPhraseToSeed)(spd);
    expect(errSPTS2).toBe(null);
    const [encData3, errEFS2] = (0, filePrivate_js_1.encryptFileSmall)(seed2, inode, revision, metadata, fileData);
    ...
    const [encFD, errFD] = (0, filePrivate_js_1.encryptFileSmall)(seed, inode, revision, metadata, fileDataAlt);
    ...
    encFD[250] += 1;
    const [, , errFD1] = (0, filePrivate_js_1.decryptFileSmall)(seed, inode, encFD);
    expect(errFD1).not.toBe(null);
    encFD[250] -= 1;
    encFD[0] += 1;
    const [, , errFD2] = (0, filePrivate_js_1.decryptFileSmall)(seed, inode, encFD);
    expect(errFD2).not.toBe(null);
    encFD[0] -= 1;
    encFD[4095] += 1;
    const [, , errFD3] = (0, filePrivate_js_1.decryptFileSmall)(seed, inode, encFD);
    expect(errFD3).not.toBe(null);
    encFD[4095] -= 1;
    // This time try withtout the modification to make sure decryption still
    // works.
    const [decFDMeta, decFDData, errFD4] = (0, filePrivate_js_1.decryptFileSmall)(seed, inode, encFD);
    ...
    const [, , errFD5] = (0, filePrivate_js_1.decryptFileSmall)(seed2, inode, encFD);
